...
#
# NAME
#       ds:agg — advanced data aggregation and statistical analysis
#
# SYNOPSIS
#       ds:agg [-h|file*] [r_aggs=expr] [c_aggs=expr] [-v option=value]
#
# DESCRIPTION
#       Aggregate and analyze data streams or files with arithmetic and
#       statistical expressions. Prefer the shell entry point `ds:agg`.
#
...
#
#       Help text lives in this file (`ds:agg -h`). Do not also load the
#       stub `agg.awk` with the modules (it is documentation-only).
#
...
#      Value extraction and truncation are cached in-process for repeated
#      field values. Extended stats compute from collected values per group.
#
# EXAMPLES
#      Basic Usage:
