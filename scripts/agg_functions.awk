function AggregationExpr(agg_expr, call, call_idx, reparse) {
    ...
    # Extended stats before mean: m(ean)? must not steal med/mode
    if (agg_expr ~ /^(med|mode|q[1-3]|sd)(\||$)/) {
        return AggregationExprExtended(agg_expr, orig_agg_expr, call, call_idx, reparse)
    }

    # Arithmetic and mean only (bare m / mean — not med/mode)
    if (agg_expr ~ /^[\+\-\*\/]/ || agg_expr ~ /^(mean|m)(\||$)/) {
        if (agg_expr ~ /^([\+\-\*\/]|mean|m)(\|all)?$/) {
            if (agg_expr ~ /^(mean|m)/) {
                mean_agg = 1
            }
        ...
        else if (agg_expr ~ /^([\+\-\*\/]|mean|m)\|(\$)?[0-9]+\.\.(\$)?[0-9]+/) {
            if (gsub(/^(mean|m)\|/, "+|", agg_expr)) {
                mean_agg = 1
            }
        ...
        else if (agg_expr ~ /^([\+\-\*\/]|mean|m)\|(\$)?[0-9]+(\|(\$)?[0-9]+(\.\.(\$)?[0-9]+)?)?$/) {
            gsub(/\$/, "", agg_expr)
            if (gsub(/^(mean|m)\|/, "+|", agg_expr)) {
                mean_agg = 1
            }
        ...
        else if (agg_expr ~ /^([\+\-\*\/]|mean|m)\|/ && agg_expr ~ /([<>=]|~)/) {
            if (gsub(/^(mean|m)\|/, "+|", agg_expr)) {
                mean_agg = 1
            }
...
# med|mode|q1-3|sd — keep AggType on the stable key (e.g. med|all); expand ranges for GenRExpr
function AggregationExprExtended(agg_expr, orig_agg_expr, call, call_idx, reparse,    type, AggBase, AggAnchor, operation, j, scope) {
    type = DetectAggType(agg_expr)
    if (!type) {
        print "Unable to parse aggregation expression " agg_expr
        exit 1
    }
    ...
    # |all: register and return stable key (GenAllAggregationExpr expands RowAggExpr)
    if (agg_expr ~ /\|all$/) {
    ...
    # Range: med|2..3 (user) or med|1..N (GenAll reparse)
    # On reparse, AggType stays on med|all; only RowAggExpr becomes 0+$1+...
    # On direct use, RA becomes the expanded form and AggType keys that form.
    if (agg_expr ~ /^(med|mode|q[1-3]|sd)\|(\$)?[0-9]+\.\.(\$)?[0-9]+$/) {
        split(agg_expr, AggBase, /\|/)
        gsub(/\$/, "", AggBase[2])
        split(AggBase[2], AggAnchor, /\.\./)
        if (!AggAnchor[1]) AggAnchor[1] = 1
        if (!AggAnchor[2]) AggAnchor[2] = call > 0 ? max_nr : 100
        agg_expr = "0+$" AggAnchor[1]
        for (j = AggAnchor[1] + 1; j <= AggAnchor[2]; j++) {
            agg_expr = agg_expr "+$" j
        }
        if (!reparse) {
            AggType[agg_expr] = type
            if (type == "quartile") {
                QuartileNum[agg_expr] = substr(orig_agg_expr, 2, 1)
            }
        }
        return agg_expr
    }

    # Cross aggregation: med|$3|$1
    if (agg_expr ~ /^(med|mode|q[1-3]|sd)\|(\$)?[0-9]+(\|(\$)?[0-9]+(\.\.(\$)?[0-9]+)?)?$/) {
        gsub(/\$/, "", agg_expr)
        sub(/^(med|mode|q[1-3]|sd)/, "+", agg_expr)
        CrossAggs[call, ++cross_agg_i] = orig_agg_expr
        XA[call, cross_agg_i] = agg_expr
        XAI[call, agg_expr] = cross_agg_i
        CrossAggExpr[call, cross_agg_i] = agg_expr
        CrossAggType[cross_agg_i] = type
        if (type == "quartile") {
            match(orig_agg_expr, /^q([1-3])/)
            QuartileNum[cross_agg_i] = substr(orig_agg_expr, RSTART+1, 1)
        }
        if (agg_expr ~ /^\+\|[0-9]+$/) {
            CrossAggForm[call, cross_agg_i] = 2
        }
    ...
    # Search: med|~pattern
    if (match(agg_expr, /^(med|mode|q[1-3]|sd)\|~/)) {
        scope = substr(agg_expr, RSTART, RLENGTH-1)  # unused; full key below
        AggType[agg_expr] = type
        if (type == "quartile") {
            QuartileNum[agg_expr] = substr(agg_expr, 2, 1)
        }
        SearchAgg[agg_expr] = substr(agg_expr, index(agg_expr, "~") + 1)
        return agg_expr
    }

    # Conditional: med|$2>1500
    if (agg_expr ~ /^(med|mode|q[1-3]|sd)\|/ && agg_expr ~ /[<>!=]/) {
        gsub(/\|\|/, "___OR___", agg_expr)
        AggType[agg_expr] = type
        if (type == "quartile") {
            QuartileNum[agg_expr] = substr(agg_expr, 2, 1)
    ...
    print "Unable to parse aggregation expression " orig_agg_expr
    exit 1
}
...
function ResolveConditionalRowAggs() {
    for (i in RA) {
        ...
        if (agg in ConditionalAgg) {
            ...
            stats_agg = (agg in AggType)
      
            for (j = 1; j <= NR; j++) {
                expr = ""
                if (stats_agg) AggValuesCount[agg, j] = 0
                ...
                for (k = 1; k <= fixed_nf; k++) {
                    ...
                    if (stats_agg) {
                        extract_val = GetOrSetExtractVal(CleanVal(f_val))
                        if (!NoVal[f_val]) {
                            AggValues[agg, j, ++AggValuesCount[agg, j]] = GetOrSetTruncVal(extract_val)
                        }
                        continue
                    }
                    if (f_val) {
                ...
                if (!stats_agg) RowAggResult[agg, j] = EvalExpr(expr)
            }
        }
    }
}
...
function GenRExpr(agg) {
    ...
    # Check if this is a mode/median/quartile/stddev aggregation that needs individual values collected
    stats_agg = (agg in AggType && (AggType[agg] == "mode" || AggType[agg] == "median" || AggType[agg] == "quartile" || AggType[agg] == "stddev"))
    ...
    if (SearchAgg[agg]) {
            ...
            if ($f ~ agg_search) {
                if (stats_agg) {
                    ...
                    if (!NoVal[val]) {
                        ...
                        AggValues[agg, NR, ++AggValuesCount[agg, NR]] = trunc_val
                    }
                }
                expr = expr "1+"
            }
        ...
        for (j = 1; j <= fs; j++) {
            ...
            if (f ~ /\$[0-9]+/) {
                ...
                val = f # Expects a static number value (e.g. sum seed 0+)
            }
            ...
            extract_val = GetOrSetExtractVal(val)
            # awk: "" == 0 — use NoVal after extract, not !extract_val
            if (NoVal[val]) continue

            trunc_val = GetOrSetTruncVal(extract_val)

            # Collect field values only for statistical aggs (skip 0+ sum seeds)
            if (stats_agg && from_field) {
                AggValues[agg, NR, ++AggValuesCount[agg, NR]] = trunc_val
            }

            expr = expr trunc_val op
...
function ResolveRowCrossAggs() {
    for (compound_i in XA) {
        ...
        for (f = 1; f <= agg_row_len; f++) {
            ...
            if (cross_agg_i in CrossAggType) {
                CrossAggValues[cross_agg_i, key_id, ++CrossAggValuesCount[cross_agg_i, key_id]] = trunc_val
            }
            else if (XAggResult[cross_agg_i, key_id]) {
