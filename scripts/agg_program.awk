...
row_column_aggs_basic {
    ...
    for (i in RA) {
        ...
        if (!RowAggResult[agg, NR]) {
            # Reset value collection for statistical aggregations (per row)
            if (agg in AggType && (AggType[agg] == "mode" || AggType[agg] == "median" || AggType[agg] == "quartile" || AggType[agg] == "stddev")) {
                AggValuesCount[agg, NR] = 0
            }
    ...
    for (i in CA) {
        ...
        else if (!KeyAgg[0, i] && !SearchAgg[agg_amort] && !Indexed(agg_amort, NR) && !(agg in AggType)) {
            continue
        }

        if (agg in AggType) {
            # Column extended stats: collect per-field values across rows
            for (f = 1; f <= fixed_nf; f++) {
                val = CleanVal($f)
                extract_val = GetOrSetExtractVal(val)
                if (NoVal[val]) continue
                trunc_val = GetOrSetTruncVal(extract_val)
                ColAggValues[agg, f, ++ColAggValuesCount[agg, f]] = trunc_val
            }
            continue
        }
...
END {
    ...
    if (!cross_aggs_only) {
        for (i = 1; i <= NR; i++) {
            ...
            for (j = 1; j <= row_aggs_count; j++) {
                ...
                print_header = i < 2 && (header || (agg in AggType && AggValuesCount[agg, i] < 1) || (!(agg in AggType) && !RowAggResult[agg, i]))
        ...
        for (i = 1; i <= column_aggs_count; i++) {
            ...
            if (header || skip_first || (agg in AggType)) {
                printf "%s", ColumnAggs[i] OFS
            }

            if (agg in AggType) {
                for (j = 1; j <= fixed_nf; j++) {
                    n = ColAggValuesCount[agg, j]
                    for (k = 1; k <= n; k++) {
    ...
    if (length(XA)) {
        ...
        for (compound_i in XA) {
            ...
            label = CrossAggs[call, cross_agg_i]
            if (!label) label = agg_op
            if (cross_agg_i in CrossAggType) {
                agg_op = CrossAggType[cross_agg_i]
            }

            if (call) {
                print "Cross Aggregation: "label" on row "agg_index" grouped by row "agg_group_scope
            }
            else {
                print "Cross Aggregation: "label" on field "agg_index" grouped by field "agg_group_scope
            }

            if (cross_agg_i in CrossAggType) {
                for (j = 1; j <= rel_len; j++) {
                    n = CrossAggValuesCount[cross_agg_i, j]
                    if (!n) continue
                    for (k = 1; k <= n; k++) {
            ...
            else if (cross_agg_i in MeanAgg) {
                for (j = 1; j <= rel_len; j++) {
